...
public:
	...
	bool send(const void*msg, size_t length); // Sends a message through buffer, if return succeed if false not enough space.
	bool recieve(char*msg);        //read a message from buffer, puts content in memory. 
