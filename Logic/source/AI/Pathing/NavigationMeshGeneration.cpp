        ...
        if (staticObj = dynamic_cast<StaticObject*> (reinterpret_cast<PhysicsObject*> (obj->getUserPointer()))) // it is a static objects
            if (!(staticObj->getNavFlags() & StaticObject::NavigationMeshFlags::CULL))                          // dont want to cull it
                if (btBoxShape* bs = dynamic_cast<btBoxShape*>(staticObj->getRigidBody()->getCollisionShape())) // and it is box shape
                {
                    ...
                    if (copy.getHalfExtentsWithoutMargin().length() > 1)
    ...
    int triangle1, int triangle2, GrowthType side)
{
    ...
    {
        if (std::abs(dot) <= 1.f)
            nav.addDoubleEdge(triangle1 * 2, triangle2 * 2, toSimple(vec2));
        else
        {
            if (dot > 0.f)
                nav.addDoubleEdge(triangle1 * 2, triangle2 * 2, toSimple((vec1 + half1 + half2 * btScalar(0.5f))));
            else
                nav.addDoubleEdge(triangle1 * 2, triangle2 * 2, toSimple((vec1 + half1 - half2 * btScalar(0.5f))));
        }
    }
    ...
    {
        if (std::abs(dot) <= 1.f)
            nav.addDoubleEdge(triangle1 * 2 + 1, triangle2 * 2 + 1, toSimple(vec2));
        else
        {
            if (dot > 0.f)
                nav.addDoubleEdge(triangle1 * 2 + 1, triangle2 * 2 + 1, toSimple((vec1 + half1 + half2 * btScalar(0.5f))));
            else
                nav.addDoubleEdge(triangle1 * 2 + 1, triangle2 * 2 + 1, toSimple((vec1 + half1 - half2 * btScalar(0.5f))));
        }
    }
